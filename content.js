...
// To add new items, copy an existing block and change the values
    ...
    if (websiteContent.skills) {
        ...
        if (skillsContainer) {
            Object.keys(websiteContent.skills).forEach(category => {
                const skillHTML = `<div class="tech-category"><h4>${category}</h4><div class="tech-tags">${websiteContent.skills[category].map(skill => `<span class="tech-tag">${skill}</span>`).join('')}</div></div>`;
    ...
    if (websiteContent.linkedinPosts && websiteContent.linkedinPosts.length > 0) {
        ...
        if (linkedinGrid) {
            ...
            websiteContent.linkedinPosts.forEach(post => {
                const postHTML = `<div class="linkedin-post"><div class="post-header"><img src="${post.avatar}" alt="${post.name}" class="post-avatar"><div class="post-author"><h4>${post.name}</h4><p>${post.position}</p><span class="post-time">${post.time}</span></div></div><div class="post-content"><p>${post.content}</p></div><div class="post-engagement"><span><i class="fas fa-thumbs-up"></i> ${post.likes}</span><span><i class="fas fa-comment"></i> ${post.comments}</span></div></div>`;
                if (firstPost) { firstPost.insertAdjacentHTML('beforebegin', postHTML); } else { linkedinGrid.insertAdjacentHTML('beforeend', postHTML); }
            });
        }
    }
    if (websiteContent.awards && websiteContent.awards.length > 0) {
        ...
        if (awardsList) {
            websiteContent.awards.forEach(award => {
                const awardHTML = `<div class="award-item"><div class="award-icon">${award.emoji}</div><div class="award-content"><h4>${award.name}</h4><p>${award.institution}${award.year ? ' • ' + award.year : ''}</p></div></div>`;
    ...
    if (websiteContent.researchInterests && websiteContent.researchInterests.length > 0) {
        ...
        if (interestsGrid) {
            websiteContent.researchInterests.forEach(interest => {
                const interestHTML = `<div class="interest-card animate-fade-in"><div class="interest-icon">${interest.icon}</div><h3>${interest.title}</h3><p>${interest.description}</p></div>`;
